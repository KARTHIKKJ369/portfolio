    ...
    <section id="github" className="py-24 md:py-32">
      <div className="container px-4">
        ...
        {status === 'error' ? (
          ...
          <Reveal delay={0.05} className="grid sm:grid-cols-3 gap-4 mb-10">
            <div className="card card-hover p-6">
              <FolderGit2 className="w-4 h-4 text-[var(--muted)] mb-3" />
              <div className="mono text-2xl font-medium">
                {stats ? stats.publicRepos : <span className="text-[var(--muted)]">···</span>}
              </div>
              <div className="text-xs text-[var(--muted)] mt-1">public repos</div>
            </div>
            <div className="card card-hover p-6">
              <Users className="w-4 h-4 text-[var(--muted)] mb-3" />
              <div className="mono text-2xl font-medium">
                {stats ? stats.followers : <span className="text-[var(--muted)]">···</span>}
              </div>
              <div className="text-xs text-[var(--muted)] mt-1">followers</div>
            </div>
            <div className="card card-hover p-6 sm:col-span-1 col-span-2">
              <div className="text-xs text-[var(--muted)] mb-3">top languages</div>
              <div className="space-y-2">
                {languages && languages.length > 0 ? (
                  languages.map((lang, i) => (
                    <div key={lang.name} className="flex items-center gap-2 text-xs">
                      <span className="w-16 text-[var(--muted)] truncate">{lang.name}</span>
                      ...
                      <span className="mono text-[var(--muted)] w-8 text-right">{lang.pct}%</span>
                    </div>
                  ))
                ) : (
